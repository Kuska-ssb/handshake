    ...
    R: Read + Unpin,
{
    fn poll_read(self: Pin<&mut Self>, cx: &mut Context, buf: &mut [u8]) -> Poll<Result<usize>> {
        ...
        debug!(
            "  poll_read reading_deciphered_buffer req {} got {}",
            buf.len(),
            len
        );
        debug!(
            "    plain_off={} plain_len={}",
            this.plain_off, this.plain_len
        );
    ...
    W: Write + Unpin,
{
    ...
    fn internal_flush(&mut self, cx: &mut Context) -> Poll<Result<()>> {
        ...
        if self.plain_len > 0 {
            ...
            encrypt_box_stream_msg(
                &mut self.key_nonce,
                MsgType::Body(&self.plain[0..self.plain_len]),
                &mut tmp_cipher,
            );
        ...
        let n = futures::ready!(
            Pin::new(&mut self.stream).poll_write(cx, self.cipher.contiguous_value())
        )?;
        self.cipher.skip(n);
    ...
    W: Write + Unpin,
{
    ...
    fn poll_write(self: Pin<&mut Self>, cx: &mut Context, buf: &[u8]) -> Poll<Result<usize>> {
        ...
        if this.plain_len < MSG_BODY_MAX_LEN {
            ...
            debug!(
                "  buffer_write dec_write {} (buf_len={})",
                consumed_bytes,
                buf.len()
            );
        }
        ...
        if this.plain_len == MSG_BODY_MAX_LEN {
            ...
            if this.cipher.cap() - this.cipher.len() >= MSG_HEADER_LEN + MSG_BODY_MAX_LEN {
                ...
                encrypt_box_stream_msg(
                    &mut this.key_nonce,
                    MsgType::Body(&this.plain[0..MSG_BODY_MAX_LEN]),
                    &mut tmp_cipher,
                );
        ...
        if this.cipher.len() > 0 {
            let polled_write =
                Pin::new(&mut this.stream).poll_write(cx, this.cipher.contiguous_value());
    ...
    fn poll_close(self: Pin<&mut Self>, cx: &mut Context) -> Poll<Result<()>> {
        ...
        if this.goodbye_off < MSG_HEADER_LEN {
            ...
            let polled_write =
                Pin::new(&mut this.stream).poll_write(cx, &tmp_cipher[this.goodbye_off..]);
